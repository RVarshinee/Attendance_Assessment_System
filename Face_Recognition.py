...
attendance_count=0
attendance_count1=0
start_date =arrow.get('2022-02-22') #Start date for academic year
#Background Image
    ...
    def TrackImages():
        ...
        if (is_number(name)) or (Id.isalpha()) or (Id not in list_string) or (is_number(stream_subject)):  #Name should be alphabetical
            ...
            while(cam.isOpened()):
                ...
                if ((attendance_count/total_days)*100) <75:
                    res="Attendance < 75%\nPlease be regular"
                    message.configure(text=res)
                ...
                def chart():
                    ...
                    import numpy as np
                    y = np.array([attendance_count,(total_days-attendance_count)])
                    mylabels = ["Attended", "Not Attended"]
                    mycolors = ["green", "blue"]
                    plt.pie(y, labels = mylabels, colors = mycolors)
                    plt.title("Attendance Status Chart")
                    plt.show()

                #Attendance status chart button
                attendance_chart=tk.Button(window,text="View Attendance\nStatus Chart",command=chart,fg="black",bg="lightpink",width=20,height=2,activebackground="green",font=('times',15,'bold'))
                attendance_chart.place(x=950,y=510)
    ...
    def TrackImages():
        ...
        if (is_number(name)) or (Id.isalpha()) or (Id not in list_string) or (is_number(designation)):  #Name should be alphabetical
            ...
            while(cam.isOpened()):
                ...
                res=attendance
                message1.configure(text=res)
                attendance_count1+=1 #Counting number of days attended
                date2=arrow.get(date)
                if (date2-start_date).days==0: #If it is the first day of reopen
                    total_days=1
                else:
                    total_days=(date2-start_date).days #Total Number of days
                attend='Number of days attended\n'+str(attendance_count1)+'/'+str(total_days)
                status=tk.Label(window,text=attend,width=20,height=2,fg="black",bg="lightpink",font=('times',15,'bold'))
                status.place(x=1200,y=730)
                if ((attendance_count/total_days)*100) <75:
                    res="Attendance < 75%\nPlease be regular"
                ...
                def chart():
                    import matplotlib.pyplot as plt
                    import numpy as np
                    y = np.array([attendance_count1,(total_days-attendance_count1)])
                    mylabels = ["Attended", "Not Attended"]
                    mycolors = ["green", "blue"]
                    plt.pie(y, labels = mylabels, colors = mycolors)
                    plt.title("Attendance Status Chart")
